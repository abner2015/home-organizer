...
export function NewItemForm({ session }: { session: ApiSession }) {
  ...
  // directly into the recommend step. This lives in an effect rather than a
  // `useState` initializer (where it used to) so the request fires only in the
  // browser: an initializer also runs during the server render, which would
  // bill a second round-trip to produce HTML nobody sees.
  const loadedRecommendFor = useRef(false);
  useEffect(() => {
    if (!recommendFor || loadedRecommendFor.current) return;
    loadedRecommendFor.current = true;
    void (async () => {
      setBusy(true);
      try {
        const it = await api.getItem(recommendFor, session);
        setItem(it);
        setForm({
          ...EMPTY_FORM,
          name: it.name,
  ...
  async function uploadAndRecognize() {
    ...
    try {
      ...
      const r = await api.recognizeItem(created.id, session);
      setVision(r.vision);
      applyInferred({
        name: r.vision.name,
        category: r.vision.category,
  ...
  async function requestRecommend() {
    ...
    try {
      ...
      const target = item
        ? await api.updateItem(item.id, body, session)
        : await api.createItem(body, session);
      setItem(target);

      const r = await api.recommend(target.id, session);
      setRec(r);
