...
#### P0.4 之后数字为何**未动**（2026-09-22 复跑确认）

P0.4 改了 ranking 的内部结构（§15.1 的类别偏好、§15.3 的理由），但 §3.2 的六项指标**逐项不变**：

| 指标 | P0.4 前 | P0.4 后 |
| --- | --- | --- |
| Total / Passed | 61 / 61 | 61 / 61 |
| Valid Slot Rate | 100% | 100% |
| Hard Violation | 1.64% | 1.64% |
| Recommendation Accuracy | 59.02% | 59.02% |
| Top-3 Recall | 70.49% | 70.49% |

原因是**权重表被原样搬进了 `score_terms()`**：`deterministic_score` 现在是
`sum(_WEIGHTS[k] * v for k, v in terms.items())`，权重在重构前后逐项相同，
所以 `det_score` 与排序**逐字节相同**。评测集里没有 `UserPreference` 行，
因此新增的 `preference` 分项恒为 0 —— 类别偏好是**反馈回路**的信号，
只有在用户真的接受过之后才会出现，评测集看不到它。

理由（`build_reason`）不进任何指标：它是**呈现层**，不参与打分与选择。
